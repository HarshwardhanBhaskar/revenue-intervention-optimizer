  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', maxWidth: '1140px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Link href="/recovery" style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', color: 'var(--color-text-secondary)' }}>
          <ArrowLeft size={14} />
          <span>Back to Opportunities Ledger</span>
        </Link>

        <div style={{ display: 'flex', gap: '10px' }}>
          <Link href={`/decision-lab?opp=${opp?.id || oppId}`} className="btn btn-secondary" style={{ padding: '6px 12px', fontSize: '12px', fontFamily: 'var(--font-mono)' }}>
          ...
          {workflowState === 'pending_approval' && (
            <>
              ...
              <button onClick={handleApprove} className="btn btn-gold" style={{ padding: '6px 16px', fontSize: '12px' }}>
                Approve & Dispatch
              </button>
            </>
          )}
      ...
      {actionSuccessMsg && (
        <div style={{ padding: '12px 16px', borderRadius: '4px', backgroundColor: 'var(--color-recovery-bg)', border: '1px solid var(--color-recovery-border)', color: 'var(--color-recovery)', fontSize: '13px' }}>
          {actionSuccessMsg}
        </div>
      )}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <span className={`badge ${workflowState === 'recovered' ? 'badge-recovery' : workflowState === 'pending_approval' ? 'badge-warning' : 'badge-info'}`}>
                {workflowState}
              </span>
            </div>
          ...
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: '10.5px', color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Recommended Action</div>
            <div style={{ fontSize: '17px', fontWeight: 700, color: 'var(--color-gold-text)', textTransform: 'uppercase', fontFamily: 'var(--font-mono)' }}>
              {recommendedAction.replace('_', ' ')}
            </div>
            <div style={{ fontSize: '12px', color: 'var(--color-recovery)', fontFamily: 'var(--font-mono)', fontWeight: 600 }}>
              +₹{Math.round(incrementalValue / 100).toLocaleString('en-IN')} net incremental gain
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr', gap: '20px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                ...
                return (
                  <div
                    ...
                    style={{
                      padding: '14px 16px',
                      borderRadius: '4px',
                      backgroundColor: isSelected ? 'var(--color-gold-bg)' : 'var(--color-surface-2)',
                      border: `1px solid ${isSelected ? 'var(--color-gold-border)' : 'var(--color-border)'}`,
                      display: 'flex',
                      alignItems: 'center',
                  ...
                  >
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <span style={{ fontWeight: 600, textTransform: 'uppercase', fontSize: '12.5px', color: isSelected ? 'var(--color-gold-text)' : 'var(--color-text-primary)' }}>
                          {rank.action_type?.replace('_', ' ')}
                        </span>
                        {isSelected && <span className="badge badge-gold">Optimal Argmax</span>}
                      </div>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontFamily: 'var(--font-mono)', fontWeight: 700, fontSize: '13.5px', color: incValue > 0 ? 'var(--color-recovery)' : 'var(--color-text-muted)' }}>
          ...
          {/* Decision Rationale: WHY THIS ACTION? WHY NOT ALTERNATIVES? */}
          <div className="card">
            <div className="card-title" style={{ marginBottom: '12px' }}>Economic Rationale & Alternative Rejections</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', fontSize: '12.5px' }}>
              <div style={{ padding: '10px 14px', borderRadius: '4px', backgroundColor: 'var(--color-surface-2)', border: '1px solid var(--color-border)' }}>
                <span style={{ fontWeight: 600, color: 'var(--color-gold-text)' }}>Why {opp.recommended_action?.replace('_', ' ').toUpperCase()} selected: </span>
                <span style={{ color: 'var(--color-text-secondary)' }}>
                  Produces the highest incremental net return (+₹{Math.round((opp.expected_incremental_value_paise || 0) / 100).toLocaleString('en-IN')}) after subtracting gateway processing fees and margin costs.
                </span>
              </div>

              <div style={{ padding: '10px 14px', borderRadius: '4px', backgroundColor: 'var(--color-surface-2)', border: '1px solid var(--color-border)' }}>
                <span style={{ fontWeight: 600, color: 'var(--color-text-muted)' }}>Why DO NOTHING rejected: </span>
                <span style={{ color: 'var(--color-text-secondary)' }}>
                  Baseline natural recovery is low ({(opp.baseline_probability * 100).toFixed(0)}%), leaving substantial revenue at risk that justifies intervention cost.
                </span>
              </div>

              <div style={{ padding: '10px 14px', borderRadius: '4px', backgroundColor: 'var(--color-surface-2)', border: '1px solid var(--color-border)' }}>
                <span style={{ fontWeight: 600, color: 'var(--color-text-muted)' }}>Why DISCOUNT rejected: </span>
                <span style={{ color: 'var(--color-text-secondary)' }}>
                  5% discount creates negligible recovery lift (+2% pts) while sacrificing ₹{Math.round(opp.amount_rupees * 0.05).toLocaleString('en-IN')} in margin.
                </span>
              </div>
            </div>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div className="card">
            <div className="card-header">
              ...
              <ShieldCheck size={16} color="var(--color-recovery)" />
            </div>
            <div style={{ fontSize: '11.5px', color: 'var(--color-text-muted)', marginBottom: '14px' }}>
              Enforced server-side without LLM or probabilistic involvement.
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {[
                ...
                >
                  ...
                  {chk.passed ? (
                    <CheckCircle2 size={15} color="var(--color-recovery)" />
                  ) : (
            ...
            {opp.actions && opp.actions.length > 0 ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '12px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <span className="badge badge-recovery">{opp.actions[0].status}</span>
                </div>
