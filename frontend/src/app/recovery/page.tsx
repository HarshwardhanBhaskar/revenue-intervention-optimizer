...
import { Search, Filter, ArrowUpDown, ChevronRight, Eye, RefreshCw, Sparkles, SlidersHorizontal } from 'lucide-react';
...
export default function RecoveryOpportunitiesPage() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', maxWidth: '1240px' }}>
      {/* Header */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span className="badge badge-gold">LEDGER</span>
            <h1 style={{ fontSize: '20px', fontWeight: 700, color: 'var(--color-text-primary)' }}>
              Recovery Opportunities Ledger
            </h1>
          </div>
          <p style={{ fontSize: '13px', color: 'var(--color-text-secondary)', marginTop: '2px' }}>
            Granular transaction-level financial diagnosis, uplift estimations, and deterministic policy status.
          </p>
        </div>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flex: 1 }}>
          <div style={{ position: 'relative', width: '280px' }}>
            ...
            <input
              type="text"
              placeholder="Search customer, reason, action..."
              value={searchQuery}
          ...
          >
            ...
            <option value="retry">Automated Retry</option>
        ...
        <div style={{ fontSize: '11.5px', color: 'var(--color-text-muted)', fontFamily: 'var(--font-mono)' }}>
            ...
            {filteredOpps.map((opp, idx) => (
              <tr key={idx}>
                ...
                <td className="font-mono" style={{ fontSize: '12px' }}>
                  ...
                  <span style={{ color: 'var(--color-recovery)', fontWeight: 600 }}>
                    {Math.round((opp.recommended_probability || 0.7) * 100)}%
                  </span>
                </td>
                <td className="font-mono" style={{ fontWeight: 600, color: (opp.expected_incremental_value_paise || 0) > 0 ? 'var(--color-recovery)' : 'var(--color-text-muted)' }}>
                  +₹{Math.round((opp.expected_incremental_value_paise || 0) / 100).toLocaleString('en-IN')}
                </td>
                <td>
                  <span className={`badge ${opp.policy_result === 'requires_human' ? 'badge-warning' : opp.policy_result === 'blocked' ? 'badge-risk' : 'badge-recovery'}`}>
                    {opp.policy_result?.replace('_', ' ') || 'approved'}
                  </span>
                </td>
                <td>
                  <span className={`badge ${opp.workflow_state === 'recovered' ? 'badge-recovery' : opp.workflow_state === 'pending_approval' ? 'badge-warning' : 'badge-muted'}`}>
